...
public class LearningProfileQueryService {
    ...
    public LearningProfile getOrCreate(Long userId) {
        var user = userRepository.findLockedById(userId)
                .orElseThrow(() -> new BusinessException("사용자를 찾을 수 없습니다.", LanguageLearningErrorCode.USER_NOT_FOUND));
        return profileRepository.findLockedByUserId(userId)
                .orElseGet(() -> profileRepository.save(LearningProfile.create(user)));
    }
    ...
    private ProfileResponseDto toResponse(Long userId, LearningProfile profile) {
        return new ProfileResponseDto(profile.getProfileVersion(), profile.getState(), profile.getBaseLevelScore(),
                profile.getCalibrationStartedDate(), profile.getCalibrationCompletedDate(), toSkillScores(profile),
                toDifficultyPerformance(profile), profile.getConfidence(), profile.getTrend(),
                getKeywordMasteries(userId),
                signalService.getResponses(userId, ProfileSignalType.GRAMMAR_WEAKNESS, PROFILE_SIGNAL_LIMIT),
                signalService.getResponses(userId, ProfileSignalType.ERROR_PATTERN, PROFILE_SIGNAL_LIMIT),
                signalService.getResponses(userId, ProfileSignalType.STRENGTH, PROFILE_SIGNAL_LIMIT),
                signalService.getResponses(userId, ProfileSignalType.WEAKNESS, PROFILE_SIGNAL_LIMIT),
                signalService.getResponses(userId, ProfileSignalType.RECOMMENDED_FOCUS, PROFILE_SIGNAL_LIMIT),
                vocabularyMasteryQueryService.get(userId));
    }

    private List<KeywordMasteryResponseDto> getKeywordMasteries(Long userId) {
        return masteryRepository.findAllByUserIdOrderByScoreAsc(userId)
                .stream()
                .limit(PROFILE_KEYWORD_LIMIT)
                .map(mastery -> new KeywordMasteryResponseDto(mastery.getCanonicalKey(), mastery.getScore(),
                        mastery.getEvaluationCount(), mastery.getSelectedCount(), mastery.getLastSelectedDate()))
                .toList();
    }

    private SkillScoresResponseDto toSkillScores(LearningProfile profile) {
        return new SkillScoresResponseDto(profile.getMeaningScore(), profile.getGrammarScore(),
                profile.getVocabularyScore(), profile.getNaturalnessScore(), profile.getExpressionScore());
    }

    private DifficultyPerformanceResponseDto toDifficultyPerformance(LearningProfile profile) {
        return new DifficultyPerformanceResponseDto(profile.getReviewPerformance(), profile.getNormalPerformance(),
    ...
    private ProfileResponseDto emptyProfile() {
        return new ProfileResponseDto("PROFILE", LearningProfileState.LEVEL_TEST_REQUIRED, null, null, null,
                ...
                new DifficultyPerformanceResponseDto(null, null, null), 0, "stable", List.of(), List.of(), List.of(),
