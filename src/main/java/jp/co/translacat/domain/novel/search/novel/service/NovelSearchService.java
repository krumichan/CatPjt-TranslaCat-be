package jp.co.translacat.domain.novel.search.novel.service;
...
public class NovelSearchService {
    ...
    @Transactional
    @SneakyThrows
    public NovelSearchPageResponseDto list(PlatformCode platformCode, String keyword, int page) {
        ...
        NovelSearchStrategy strategy = this.strategy(platformCode)
                .orElseThrow(() -> new EntityNotFoundException("지원하지 않는 플랫폼입니다: " + platformCode));

        String url = strategy.getUrl(urlTemplate.getUrlPattern(), keyword, page);

        NovelSearchContext scrapped = strategy.scrape(url);
        List<NovelContext> scrappedNovels = scrapped.getNovelContexts();
        List<String> identifiers = scrappedNovels.stream().map(NovelContext::getIdentifier).toList();
        ...
        // 번역이 필요한 유닛들만 추출.
        for (NovelContext ctx : scrappedNovels) {
        ...
        // 번역된 내용 DB 저장 또는 갱신.
        this.novelSafeSaver.saveNovels(platform, scrappedNovels);

        return NovelSearchPageResponseDto.of(scrapped.getPageNumberContext(), scrappedNovels);
    }
}
