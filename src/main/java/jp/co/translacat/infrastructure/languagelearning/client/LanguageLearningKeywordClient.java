...
public class LanguageLearningKeywordClient {
    ...
    public LanguageLearningKeywordClient(RestClient client, LanguageLearningInternalJwtProvider jwt, ObjectMapper json) {
        this.client = client;
        this.jwt = jwt;
    ...
    public KeywordResponseDto updateCustom(Long userId, boolean started, Long keywordId, KeywordUpdateRequestDto request) {
        requireId(keywordId);
    ...
    public KeywordResponseDto selectSystem(Long userId, boolean started, Long keywordId, boolean selected) {
        ...
        return read(() -> client.put().uri(USER + "/system/" + keywordId + "/selection")
                .header(HttpHeaders.AUTHORIZATION, bearer(jwt.issueKeywordUserToken(userId, started)))
                .body(new SystemKeywordSelectionRequestDto(selected)).retrieve().body(byte[].class), KeywordResponseDto.class);
    }
    ...
    public List<KeywordCandidateSnapshot> candidates(Long userId, boolean started, LocalDate date) {
        ...
        return read(() -> client.get().uri(uri -> uri.path(USER + "/candidates").queryParam("learningDate", date).build())
                .header(HttpHeaders.AUTHORIZATION, bearer(jwt.issueKeywordUserToken(userId, started)))
    ...
    private <T> T read(Call call, Class<T> type) {
        try {
            ...
            throw new LanguageLearningServiceException(HttpStatus.BAD_GATEWAY, "LL_SERVICE_UNAVAILABLE", "키워드 서비스에 연결할 수 없습니다.", failure);
        } catch (IOException | RestClientException | IllegalArgumentException failure) {
            throw new LanguageLearningServiceException(HttpStatus.BAD_GATEWAY, "LL_SERVICE_RESPONSE_ERROR", "키워드 서비스 응답을 처리할 수 없습니다.", failure);
        }
    }
    private static void validate(Object value) {
        if (value == null) throw invalidResponse();
        if (value instanceof KeywordResponseDto row) {
            if (row.id() == null || row.id() <= 0 || row.text() == null || row.type() == null || row.source() == null || row.sortOrder() < 0) throw invalidResponse();
            ...
            if (row.key() == null || row.text() == null || row.type() == null || row.source() == null || row.availableFrom() == null) throw invalidResponse();
        }
    }
    private InternalApiErrorDto readError(byte[] bytes) {
        if (bytes == null || bytes.length == 0 || bytes.length > 65_536) return null;
        try { return json.readValue(bytes, InternalApiErrorDto.class); }
    ...
    private static LanguageLearningServiceException invalidResponse() {
        return new LanguageLearningServiceException(HttpStatus.BAD_GATEWAY, "LL_KEYWORDS_CONTRACT_ERROR", "키워드 응답 계약이 유효하지 않습니다.");
    }
    private static void requireId(Long id) {
        if (id == null || id <= 0) throw new IllegalArgumentException("키워드 ID는 양수여야 합니다.");
    }
    private static String bearer(String token) { return "Bearer " + token; }
    @FunctionalInterface private interface Call { byte[] execute(); }
}
