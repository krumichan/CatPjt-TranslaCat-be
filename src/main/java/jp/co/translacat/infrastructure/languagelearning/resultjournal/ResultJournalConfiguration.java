...
public class ResultJournalConfiguration {
    @Bean public ResultOutboxStore resultOutboxStore(JdbcTemplate jdbc, PlatformTransactionManager manager, ResultDeliveryProperties properties) {
        properties.validate();
        var store = new ResultOutboxStore(jdbc, manager, Clock.systemUTC());
        if (properties.isEnabled()) store.verifySchema();
        return store;
    }
    @Bean public ResultJournalListener resultJournalListener(ResultOutboxStore store, ResultDeliveryProperties properties) {
        return new ResultJournalListener(store, properties);
    }
    @Bean public ResultOutboxDispatcher resultOutboxDispatcher(ResultOutboxStore store, ResultDeliveryProperties properties,
