...
public class ResultOutboxStore {
    ...
    public void verifySchema() {
        jdbc.queryForList(
                "SELECT source_instance_id, user_id, last_sequence FROM language_learning_result_outbox_stream WHERE 1=0");
        jdbc.queryForList(
                "SELECT event_id, schema_version, source_instance_id, user_id, stream_sequence, kind, reference_id, occurred_at, payload_json, payload_sha256, attempts, blocked, claim_token, lease_until, next_attempt_at, delivered_at, last_error_code FROM language_learning_result_outbox WHERE 1=0");
    }
    ...
    public ResultEnvelope append(String sourceId, LearningResultCaptured fact) {
        ...
        jdbc.update(
                "INSERT INTO language_learning_result_outbox_stream(source_instance_id,user_id,last_sequence) VALUES(?,?,0) "
        ...
        jdbc.update(
                "INSERT INTO language_learning_result_outbox(event_id,schema_version,source_instance_id,user_id,stream_sequence,kind,"
                        + "reference_id,occurred_at,payload_json,payload_sha256,attempts,blocked,next_attempt_at) "
                        + "VALUES(?,1,?,?,?,?,?,?,?,?,0,FALSE,?)",
                event.eventId(), sourceId, event.userId(), sequence, event.kind(), event.referenceId(),
                event.occurredAt(),
                event.payloadJson(), hash, Timestamp.from(now));
        jdbc.update(
                "UPDATE language_learning_result_outbox_stream SET last_sequence=? WHERE source_instance_id=? AND user_id=?",
    ...
    public Optional<Claim> claim(String sourceId, int leaseSeconds, int maximumAttempts) {
        ...
        return Objects.requireNonNull(isolated.execute(status -> {
            ...
            List<String> candidates = jdbc.queryForList("SELECT e.event_id FROM language_learning_result_outbox e "
                            ...
                            + "ORDER BY e.next_attempt_at,e.event_id LIMIT 20", String.class, sourceId, Timestamp.from(now),
                    Timestamp.from(now));
            for (String id : candidates) {
                // 매번 응답 전에 프로세스가 종료되어도 시도 상한을 무한히 넘기지 않는다.
                int exhausted = jdbc.update(
                        "UPDATE language_learning_result_outbox SET blocked=TRUE,last_error_code='ATTEMPTS_EXHAUSTED',"
                ...
                int updated = jdbc.update(
                        "UPDATE language_learning_result_outbox SET claim_token=?,lease_until=?,attempts=attempts+1 "
    ...
    private ResultEnvelope envelope(ResultSet rs) throws SQLException {
        return new ResultEnvelope(rs.getInt("schema_version"), rs.getString("source_instance_id"),
                rs.getString("event_id"), rs.getLong("user_id"),
                rs.getLong("stream_sequence"), rs.getString("kind"), rs.getString("reference_id"),
    ...
    public record Claim(ResultEnvelope event, String token, int attempts) {
        ...
        public String toString() {
            return "ResultClaim(eventId=" + event.eventId() + ", attempts=" + attempts + ")";
        }
    }
}
