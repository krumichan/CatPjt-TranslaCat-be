...
class ReceiptAnalysisFacadeTest {
    ...
    @Test void malformedAmountsDatesAndCategoryAreReviewableWithoutAbort() {
        when(ai.callReceiptAnalysis(any(),any())).thenReturn(new AiReceiptAnalysisResponse(List.of(
        ...
        assertThat(result.receipts().getFirst().warnings()).contains("INVALID_AMOUNT","INVALID_DATE","CATEGORY_REQUIRES_REVIEW");
        assertThat(result.receipts().getFirst().categoryName()).isNull();
