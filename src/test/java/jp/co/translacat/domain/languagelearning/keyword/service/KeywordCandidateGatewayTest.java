...
class KeywordCandidateGatewayTest {
    @Test void llCandidateUsesExistingCoreMasteryAndWeightPolicy() {
        ...
        when(catalog.candidates(123L, date)).thenReturn(List.of(new KeywordCandidateSnapshot("CUSTOM:8", "IT", KeywordSource.CUSTOM, KeywordType.TOPIC, "it", date)));
    ...
    @Test void absentMasteryKeepsOriginalRampAndRecencyDefaults() {
        var catalog = mock(KeywordCatalogGateway.class);
        var mastery = mock(KeywordMasteryRepository.class);
        var date = LocalDate.of(2026, 9, 24);
        when(catalog.candidates(123L, date)).thenReturn(List.of(new KeywordCandidateSnapshot("SYSTEM:1", "IT", KeywordSource.SYSTEM, KeywordType.TOPIC, null, date)));
        when(mastery.findByUserIdAndCanonicalKey(123L, "it")).thenReturn(Optional.empty());
        var rows = new KeywordCandidateQueryService(catalog, mastery, new KeywordSelectionWeightPolicy()).findCandidates(123L, date);
    ...
    @Test void emptyCatalogDoesNotInventFallbackKeywordsOrMastery() {
        var catalog = mock(KeywordCatalogGateway.class);
        var mastery = mock(KeywordMasteryRepository.class);
        var date = LocalDate.of(2026, 9, 24);
        when(catalog.candidates(123L, date)).thenReturn(List.of());
        assertTrue(new KeywordCandidateQueryService(catalog, mastery, new KeywordSelectionWeightPolicy()).findCandidates(123L, date).isEmpty());
