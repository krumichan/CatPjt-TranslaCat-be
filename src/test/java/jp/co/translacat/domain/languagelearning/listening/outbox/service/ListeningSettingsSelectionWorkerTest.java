...
import jp.co.translacat.infrastructure.languagelearning.client.dto.SelectionDeliveryResponseDto;
...
class ListeningSettingsSelectionWorkerTest {
    ...
    private ListeningOutboxTransactionService.ClaimedEvent event() {
        return new ListeningOutboxTransactionService.ClaimedEvent(91L, ListeningOutboxType.REMEMBER_SETTINGS_SELECTION,
                81L,
                json.write(new SettingsSelectionPayload(123L, "2026-09-24T01:00:00.123456",
                        List.of(ListeningTaskType.SUMMARY))),
                "settings-selection:session:81", 1);
    }

    @Test
    void lostClaimDoesNotDeliverAnything() {
        worker.process(event());
        verifyNoInteractions(access, client);
    }

    @Test
    void acknowledgementOnlyCompletesTheOwnedAttempt() {
        var event = event();
        when(tx.ownsClaim(event)).thenReturn(true);
        when(access.forUser(123L)).thenReturn(client);
        when(client.rememberListeningSelection(eq(123L), any())).thenReturn(
                new SelectionDeliveryResponseDto("DUPLICATE"));
        worker.process(event);
        verify(client).rememberListeningSelection(eq(123L),
                argThat(dto -> dto.eventId() == 91L && dto.expectedRevision().equals("2026-09-24T01:00:00.123456")));
        verify(tx).succeedIfOwned(eq(event), any());
        verify(tx, never()).succeed(anyLong(), any());
    }

    @Test
    void transientFailureRetainsTheDurableEventForRetry() {
        var event = event();
        when(tx.ownsClaim(event)).thenReturn(true);
        when(access.forUser(123L)).thenReturn(client);
        when(client.rememberListeningSelection(eq(123L), any())).thenThrow(
                new LanguageLearningServiceException(HttpStatus.BAD_GATEWAY, "LL_SERVICE_UNAVAILABLE", "민감한 원문"));
        worker.process(event);
    ...
    @Test
    void permanentAuthorizationFailureIsNotRetriedBlindly() {
        var event = event();
        when(tx.ownsClaim(event)).thenReturn(true);
        when(access.forUser(123L)).thenReturn(client);
        when(client.rememberListeningSelection(eq(123L), any())).thenThrow(
                new LanguageLearningServiceException(HttpStatus.FORBIDDEN, "LEARNER_UNAVAILABLE", "정지"));
        worker.process(event);
        verify(tx).fail(eq(event), eq("LEARNER_UNAVAILABLE"), eq(false), any(), eq(100), any());
    }

    @Test
    void coreLookupConnectionFailureIsRetryableRatherThanInvalidPayload() {
        var event = event();
        when(tx.ownsClaim(event)).thenReturn(true);
        when(access.forUser(123L)).thenThrow(
                new org.springframework.dao.DataAccessResourceFailureException("DB unavailable"));
        worker.process(event);
    ...
    @Test
    void ambiguousCoreAcknowledgementCanBeRetriedWithTheSameEventId() {
        var event = event();
        when(tx.ownsClaim(event)).thenReturn(true);
        when(access.forUser(123L)).thenReturn(client);
        when(client.rememberListeningSelection(eq(123L), any())).thenReturn(
                new SelectionDeliveryResponseDto("APPLIED"));
    ...
    @Test
    void workerSuspendsAnyAccidentalCallerTransaction() throws Exception {
        var annotation = ListeningSettingsSelectionWorker.class.getMethod("process",
                        ListeningOutboxTransactionService.ClaimedEvent.class)
