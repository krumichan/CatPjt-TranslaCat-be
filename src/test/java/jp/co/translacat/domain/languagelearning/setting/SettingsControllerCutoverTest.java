...
class SettingsControllerCutoverTest {
    ...
    static class ConfigurationForTest {
        @Bean
        AdminSettingsGateway gateway() {
            return mock(AdminSettingsGateway.class);
        }

        @Bean
        AdminLanguageLearningSettingFacade facade(AdminSettingsGateway gateway) {
            return new AdminLanguageLearningSettingFacade(gateway);
        }

        @Bean
        AdminLanguageLearningSettingController controller(AdminLanguageLearningSettingFacade facade) {
            return new AdminLanguageLearningSettingController(facade);
        }
    }

    private UserPrincipal principal() {
        var principal = mock(UserPrincipal.class);
        when(principal.getId()).thenReturn(987L);
        return principal;
    }

    @Test
    void userGetPreservesTheExternalEnvelopeAndAuthenticatedId() {
        var gateway = mock(UserSettingsGateway.class);
    ...
    @Test
    void nonAdminCannotInvokeAdminReadEvenThroughTheSpringProxy() {
        try (var context = new AnnotationConfigApplicationContext(ConfigurationForTest.class)) {
            ...
            assertThrows(AccessDeniedException.class,
                    () -> context.getBean(AdminLanguageLearningSettingController.class).get(principal));
    ...
    @Test
    void adminReadUsesTheRealAuthenticatedIdAndOneEnvelope() {
        try (var context = new AnnotationConfigApplicationContext(ConfigurationForTest.class)) {
            var principal = principal();
            ...
            assertEquals(200, response.getResultCode());
            assertEquals(5, response.getBody().defaultDailySentenceCount());
