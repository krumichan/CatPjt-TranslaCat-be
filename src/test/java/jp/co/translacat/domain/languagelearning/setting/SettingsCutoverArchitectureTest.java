...
class SettingsCutoverArchitectureTest {
    ...
    @Test void legacySettingsImplementationsAndMappingsAreGone() throws Exception {
        var forbidden = List.of("LanguageLearningUserSettingQueryService", "LanguageLearningAdminSettingQueryService",
                "ListeningPolicySettingQueryService", "LanguageLearningUserSettingRepository", "LanguageLearningAdminSettingRepository",
        ...
        try (var paths = Files.walk(root)) {
            for (Path path : paths.filter(p -> p.toString().endsWith(".java")).toList()) {
                ...
                for (String table : List.of("language_learning_user_setting", "language_learning_admin_setting", "language_learning_admin_setting_audit", "language_learning_listening_policy_setting")) {
                    assertFalse(text.contains("@Table(name = \"" + table + "\""), path.toString());
                }
            }
        }
    }
    @Test void snapshotModelsHaveNoJpaOrSpringDependency() throws Exception {
        for (Path folder : List.of(root.resolve("domain/languagelearning/setting/model"), root.resolve("domain/languagelearning/listening/setting/model"))) {
            try (var paths = Files.walk(folder)) {
                for (Path path : paths.filter(p -> p.toString().endsWith(".java")).toList()) {
                    var text = Files.readString(path);
                    assertFalse(text.contains("import jakarta.persistence.")); assertFalse(text.contains("import org.springframework."));
                    assertFalse(text.contains("createDefault("));
                }
            }
        }
    }
    @Test void externalSettingsPathsAndAdminAuthorizationRemainUnchanged() throws Exception {
        var controllers = root.resolve("domain/languagelearning/setting/controller");
        assertTrue(Files.readString(controllers.resolve("LanguageLearningSettingController.java")).contains("/api/v1/language-learning/settings"));
