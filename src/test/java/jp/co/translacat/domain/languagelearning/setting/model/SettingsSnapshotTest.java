...
class SettingsSnapshotTest {
    @Test
    void allAdminPolicyAccessorsPreserveTheReceivedValues() throws Exception {
        ...
        for (var field : AdminSettingResponseDto.class.getRecordComponents()) {
            ...
            assertEquals(field.getAccessor().invoke(snapshot.settings()),
                    AdminSettingsSnapshot.class.getMethod(method).invoke(snapshot));
        }
    }

    @Test
    void allUserSettingAccessorsPreserveTheReceivedValues() throws Exception {
        ...
        for (var field : UserSettingResponseDto.class.getRecordComponents()) {
            ...
            String method = (field.getType() == boolean.class ? "is" : "get")
                    + Character.toUpperCase(field.getName().charAt(0)) + field.getName().substring(1);
            assertEquals(field.getAccessor().invoke(snapshot.settings()),
                    UserSettingsSnapshot.class.getMethod(method).invoke(snapshot));
        }
        assertEquals("[\"DICTATION\"]", snapshot.getDefaultListeningTaskTypesJson());
    }

    @Test
    void mutableInputListCannotChangeTheSnapshotLater() {
    ...
    @Test
    void snapshotsRejectMissingIdentityRevisionAndPolicy() {
        var value = SettingsSnapshotFixtures.user(123);
        assertThrows(IllegalArgumentException.class,
                () -> new UserSettingsSnapshot(0L, value.learningDate(), value.revision(), value.settings()));
        assertThrows(NullPointerException.class,
                () -> new UserSettingsSnapshot(123L, null, value.revision(), value.settings()));
        assertThrows(NullPointerException.class,
                () -> new UserSettingsSnapshot(123L, value.learningDate(), null, value.settings()));
        assertThrows(NullPointerException.class, () -> new AdminSettingsSnapshot(null));
    }

    @Test
    void pendingDatesAndTargetsAreNotRecomputedInBe() {
    ...
    @Test
    void listeningPolicyUsesRemoteLimitsWithoutLocalDefaults() {
    ...
    @Test
    void adminClampingUsesOnlyThisReceivedPolicy() {
