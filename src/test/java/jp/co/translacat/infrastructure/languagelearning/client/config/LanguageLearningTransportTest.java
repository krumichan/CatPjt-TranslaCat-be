...
class LanguageLearningTransportTest {
    ...
    private LanguageLearningClientProperties properties(int port) {
        ...
        p.setUrl("http://127.0.0.1:" + port);
        p.getInternalJwt().setSecretBase64(Base64.getEncoder().encodeToString(new byte[32]));
        return p;
    }

    @Test
    void actualJdkTransportSendsPatchAndJwt() throws Exception {
        ...
        try {
            ...
            var result = client.updateUserSettings(123L,
                    new UserSettingUpdateRequestDto(null, null, null, 7, null, null, null, null, null));
            assertEquals("PATCH", method.get());
            assertTrue(bearer.get().startsWith("Bearer "));
    ...
    @Test
    void redirectNeverForwardsBearerToAnotherPath() throws Exception {
        var server = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
        ...
        try {
            var p = properties(server.getAddress().getPort());
            var client = new LanguageLearningSettingsClient(
                    new LanguageLearningClientConfiguration().languageLearningRestClient(RestClient.builder(), p),
                    new LanguageLearningInternalJwtProvider(p, Clock.systemUTC()), mapper);
    ...
    @Test
    void credentialsAndUnexpectedBasePathsAreRejected() {
        var p = properties(8081);
        for (String value : new String[]{
                "http://user:pass@localhost:8081",
                "http://localhost:8081/api",
                "http://localhost:8081?key=secret",
                "http://example.test:8081"
        }) {
            p.setUrl(value);
            assertThrows(IllegalStateException.class, () -> LanguageLearningClientConfiguration.validate(p));
        }
        p.setUrl("https://ll.example.test");
        LanguageLearningClientConfiguration.validate(p);
    }
}
