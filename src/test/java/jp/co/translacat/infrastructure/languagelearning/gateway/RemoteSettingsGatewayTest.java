...
class RemoteSettingsGatewayTest {
    ...
    @Test void disabledClientFailsClosedWithoutCoreSettingsFallback() {
    ...
    @Test void missingCoreUserNeverCreatesRemoteLearner() {
        var gateway = new RemoteUserSettingsGateway(access());
        assertThrows(BusinessException.class, () -> gateway.getSnapshot(999L));
        verifyNoInteractions(client);
    }
    @Test void synchronizedSnapshotCarriesPostPromotionDateAndRevision() {
        var gateway = new RemoteUserSettingsGateway(access());
    ...
    @Test void mismatchedUserResponseIsNotAccepted() {
        var gateway = new RemoteUserSettingsGateway(access());
    ...
    @Test void passiveDateDoesNotReadOrCreateUserSettings() {
        var gateway = new RemoteUserSettingsGateway(access());
    ...
    @Test void remoteErrorDoesNotTryAnotherReadSource() {
        var gateway = new RemoteUserSettingsGateway(access());
    ...
    @Test void userUpdateUsesActualUserAndPreservesResponse() {
        var gateway = new RemoteUserSettingsGateway(access());
    ...
    @Test void backgroundAdminReadDoesNotInventAnAdminIdentity() {
        ...
        verify(client).getAdminPolicy(); verify(client, never()).getAdminSettings(any());
        verifyNoInteractions(users);
    }
    @Test void publicAdminReadUsesTheAuthenticatedCoreIdentity() {
        var gateway = new RemoteAdminSettingsGateway(access());
        when(client.getAdminSettings(123L)).thenReturn(SettingsSnapshotFixtures.admin().settings());
        gateway.getSettings(123L);
        verify(client).getAdminSettings(123L); verify(users).existsById(123L);
    }
    @Test void languagePairResultsAreImmutableAndReadThroughTheServiceContract() {
        var gateway = new RemoteUserSettingsGateway(access());
