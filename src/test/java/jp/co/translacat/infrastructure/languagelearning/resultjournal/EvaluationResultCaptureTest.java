...
class EvaluationResultCaptureTest {
    ...
    private class SpeakingFixture {
        ...
        final SpeakingEvaluationResultCommandService service = new SpeakingEvaluationResultCommandService(
                turns, evaluations, metrics, new SpeakingEvaluationEligibilityPolicy(), codec, events, activities, history, profile);
        SpeakingFixture() {
            var user = mock(User.class); when(user.getId()).thenReturn(123L);
            when(session.getUser()).thenReturn(user); when(session.getId()).thenReturn(51L);
            ...
            when(session.getOriginLanguage()).thenReturn("ko"); when(session.getLearningLanguage()).thenReturn("ja");
            ...
            when(evaluation.getId()).thenReturn(61L); when(evaluation.getEvaluationVersion()).thenReturn("test-v1");
            when(turns.findAllBySessionIdOrderByTurnIndexAsc(51L)).thenReturn(List.of());
            when(activities.findBySourceAndReferenceId(LearningSource.SPEAKING, "51")).thenReturn(Optional.of(activity));
        ...
        LearningResultCaptured capture() {
            ...
            verify(events).publishEvent(captor.capture()); return captor.getValue();
        }
    }
    @Test void scoredSpeakingPublishesAfterProfileAndActivityAreUpdated() throws Exception {
        var f = new SpeakingFixture(); var response = f.response(0.9); f.service.apply(f.session, response);
        ...
        var event = f.capture(); var json = mapper.readTree(event.payloadJson());
        assertEquals("SPEAKING_SCORED", event.kind()); assertEquals("51", event.referenceId());
        assertTrue(json.get("formal").booleanValue()); assertEquals(0.9, json.get("activityWeight").doubleValue());
    }
    @Test void insufficientSpeakingHasNoProfileUpdateAndIsNotMarkedAsFormal() throws Exception {
        var f = new SpeakingFixture(); f.service.apply(f.session, f.response(0.2));
        verifyNoInteractions(f.profile, f.history);
        var event = f.capture(); var json = mapper.readTree(event.payloadJson());
        assertEquals("SPEAKING_INSUFFICIENT", event.kind()); assertFalse(json.get("formal").booleanValue());
        assertEquals(0.0, json.get("activityWeight").doubleValue());
    }
    @Test void freeCoachingCannotUseTheScoredPath() {
        var f = new SpeakingFixture(); when(f.session.getResultKind()).thenReturn(SpeakingResultKind.SESSION_COACHING);
    ...
    @Test void anAlreadyStoredSpeakingResultDoesNotPublishAgain() {
        var f = new SpeakingFixture(); when(f.evaluation.getStatus()).thenReturn("EVALUATED");
    ...
    private class WritingFixture {
        ...
        WritingFixture() {
            when(user.getId()).thenReturn(123L); when(answer.getId()).thenReturn(31L);
            when(answer.getDailyItem()).thenReturn(item); when(item.getId()).thenReturn(41L);
            when(item.getDifficulty()).thenReturn(DailyWritingDifficulty.NORMAL);
            when(evaluation.getId()).thenReturn(21L); when(evaluations.findByAnswerId(31L)).thenReturn(Optional.of(evaluation));
            when(settings.getOriginLanguage()).thenReturn("ko"); when(settings.getLearningLanguage()).thenReturn("ja");
            when(requests.createDaily(answer, settings, snapshot, day)).thenReturn(new WritingEvaluationRequestContext(null, List.of()));
            when(ai.evaluate(null)).thenReturn(response);
        }
        void execute() { service.evaluateDaily(user, answer, settings, snapshot, day); }
    }
    @Test void writingPublishesOnlyAfterTheExistingProfileUpdate() throws Exception {
        var f = new WritingFixture(); f.execute();
        ...
        order.verify(f.events).publishEvent(any(LearningResultCaptured.class));
        var capture = ArgumentCaptor.forClass(LearningResultCaptured.class); verify(f.events).publishEvent(capture.capture());
        var event = capture.getValue(); assertEquals("WRITING_SCORED", event.kind());
        assertEquals(31L, mapper.readTree(event.payloadJson()).get("answerId").longValue());
    }
    @Test void failedWritingDoesNotPublishASuccessfulResult() {
        var f = new WritingFixture(); when(f.ai.evaluate(null)).thenThrow(new IllegalStateException("test AI failure"));
