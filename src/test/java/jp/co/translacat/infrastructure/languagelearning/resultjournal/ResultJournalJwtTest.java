...
class ResultJournalJwtTest {
    @Test void resultTokenHasOnlyItsOwnPurposeAndScope() {
        byte[] key = new byte[32]; Instant now = Instant.parse("2026-09-24T03:00:00Z");
        var properties = new LanguageLearningClientProperties(); properties.getInternalJwt().setSecretBase64(Encoders.BASE64.encode(key));
        var provider = new LanguageLearningInternalJwtProvider(properties, Clock.fixed(now, ZoneOffset.UTC));
        var claims = Jwts.parser().clock(() -> Date.from(now)).verifyWith(Keys.hmacShaKeyFor(key)).requireIssuer("translacat-be")
                .requireAudience("translacat-ll").build().parseSignedClaims(provider.issueLearningResultsToken()).getPayload();
