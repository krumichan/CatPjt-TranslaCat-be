...
@DataJpaTest(properties = "spring.datasource.url=jdbc:h2:mem:result-journal-tests;MODE=MySQL;DB_CLOSE_DELAY=-1;NON_KEYWORDS=USER")
...
class ResultOutboxStoreJpaTest {
    ...
    @Autowired ResultOutboxStore store;
    @Autowired ApplicationEventPublisher publisher;
    @Autowired UserRepository users;
    private static final String SOURCE = ResultDeliveryRulesTest.SOURCE;

    @TestConfiguration static class Config {
        @Bean ResultDeliveryProperties journalProperties() {
            var p = new ResultDeliveryProperties(); p.setEnabled(true); p.setSourceInstanceId(SOURCE); return p;
        }
        @Bean ResultOutboxStore resultStore(JdbcTemplate jdbc, PlatformTransactionManager manager) {
            return new ResultOutboxStore(jdbc, manager, Clock.systemUTC());
        }
        @Bean ResultJournalListener journalListener(ResultOutboxStore store, ResultDeliveryProperties p) {
    ...
    private ResultEnvelope append(long id) { return new TransactionTemplate(manager).execute(s -> store.append(SOURCE, fact(id))); }
    private long count() { return jdbc.queryForObject("SELECT COUNT(*) FROM language_learning_result_outbox", Long.class); }
    private ResultAcknowledgement ack(ResultOutboxStore.Claim claim) {
        var e = claim.event(); return new ResultAcknowledgement(e.sourceInstanceId(), e.eventId(), e.userId(), e.sequence(), e.payloadSha256(), "RECORDED");
    }
    @Test void beforeCommitRecordsTheFactInTheSourceTransaction() {
        new TransactionTemplate(manager).executeWithoutResult(s -> {
            publisher.publishEvent(fact(123)); assertEquals(0L, count());
        });
        assertEquals(1L, count());
    }
    @Test void sourceRollbackAlsoRollsBackOutboxAndSequence() {
        String email = UUID.randomUUID() + "@result-journal.test";
        new TransactionTemplate(manager).executeWithoutResult(s -> {
            var user = users.saveAndFlush(User.createLocalUser(email, "test", "test", Role.USER, UUID.randomUUID().toString().substring(0, 20)));
            publisher.publishEvent(fact(user.getId())); s.setRollbackOnly();
        });
        assertEquals(0L, count());
        assertEquals(0L, jdbc.queryForObject("SELECT COUNT(*) FROM `user` WHERE email=?", Long.class, email));
    }
    @Test void outboxFailureRollsBackSourceJpaWriteRatherThanLosingTheEvent() {
    ...
    @Test void appendWithoutSourceTransactionIsRejected() {
        assertThrows(IllegalStateException.class, () -> store.append(SOURCE, fact(123)));
    }
    @Test void committedSequencesAreContiguousAfterRollback() {
        append(123);
        new TransactionTemplate(manager).executeWithoutResult(s -> { store.append(SOURCE, fact(123)); s.setRollbackOnly(); });
        assertEquals(2L, append(123).sequence());
    }
    @Test void oneStreamWaitsForAcknowledgementButAnotherUserCanAdvance() {
        append(123); append(123); append(124);
        ...
        store.acknowledge(first, ack(first)); store.acknowledge(secondUser, ack(secondUser));
        assertEquals(2L, store.claim(SOURCE, 60, 10).orElseThrow().event().sequence());
    }
    @Test void expiredLeaseIsReclaimedWithANewTokenAndStaleWorkerCannotAcknowledge() {
        append(123);
        var stale = store.claim(SOURCE, 60, 10).orElseThrow();
        jdbc.update("UPDATE language_learning_result_outbox SET lease_until=?", Timestamp.from(Instant.now().minusSeconds(60)));
    ...
    @Test void blockedHeadIsNotSkippedAndResponseMismatchNeverMarksDelivery() {
        append(123); append(123);
        var claim = store.claim(SOURCE, 60, 10).orElseThrow();
        var mismatch = new ResultAcknowledgement(SOURCE, claim.event().eventId(), 123, 9, claim.event().payloadSha256(), "RECORDED");
    ...
    @Test void repeatedCrashesEventuallyBlockTheHeadWithoutSkippingIt() {
        append(123); append(123);
        ...
        assertEquals("ATTEMPTS_EXHAUSTED", jdbc.queryForObject(
                "SELECT last_error_code FROM language_learning_result_outbox WHERE event_id=?", String.class, first.event().eventId()));
        assertTrue(store.claim(SOURCE, 60, 1).isEmpty());
    }
}
